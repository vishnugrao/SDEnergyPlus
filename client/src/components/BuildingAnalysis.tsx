...
import { AnalysisResult, CityAnalysisResults } from '@/types/analysis';
...
// Cache for all buildings analysis
let allBuildingsCache: {
    timestamp: number;
    results: CityAnalysisResults;
} | null = null;

const CACHE_DURATION = 5 * 60 * 1000; // 5 minutes in milliseconds

export function BuildingAnalysis({ buildingIds }: BuildingAnalysisProps) {
    ...
    useEffect(() => {
        const fetchBuildings = async () => {
            try {
                ...
                // If no specific IDs are provided, fetch all buildings
    ...
    useEffect(() => {
        const fetchAnalysisResults = async () => {
            ...
            try {
                ...
                // Check if we can use the cache for all buildings analysis
                const isAllBuildingsAnalysis = buildingIds.length === 0;
                const now = Date.now();
                const canUseCache = isAllBuildingsAnalysis && 
                    allBuildingsCache !== null && 
                    (now - allBuildingsCache.timestamp) < CACHE_DURATION;

                if (canUseCache && allBuildingsCache !== null) {
                    setAnalysisResults(allBuildingsCache.results);
                    setAnalysisLoading(false);
                    return;
                }
                ...
                // Update cache if this is an all buildings analysis
                if (isAllBuildingsAnalysis) {
                    allBuildingsCache = {
                        timestamp: now,
                        results
                    };
    ...
    // Listen for new design saves to invalidate cache
    useEffect(() => {
        const handleNewDesign = () => {
            invalidateCache();
        };

        window.addEventListener('newDesignSaved', handleNewDesign);
        return () => {
            window.removeEventListener('newDesignSaved', handleNewDesign);
        };
    }, []);
    ...
    return (
        <div className="container mx-auto py-8 space-y-6">
            ...
            <BuildingPairwiseComparison buildings={getCurrentCityData()} />
        </div>
    );
} 